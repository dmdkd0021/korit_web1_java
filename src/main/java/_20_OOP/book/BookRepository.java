...
public class BookRepository {
    ...
    // null? 참조자료형에만 있는 값
    // 참조자료형 변수에 heap 주소가 저장되어야 하는데, 주소가 없는 경우가 있음(초기화가 안된 것)
    // 저장된 주소가 없습니다를 표현하는 값
    // NullPointerException -> 참조자료형은 .으로 참조할 수 있는데 초기화가 안되면 null을 참조하게 되어 예외가 발생
    int getEmptyIndex() {
        ...
        return -1; // 0을 포함한 양수가 아닌 경우
    }

    // 등록된 도서들을 조회
    // 배열에 5칸을 만들어놨는데, 만약 2권만 등록됐다면? 오류나겟지머...
    // 배열의 길이만큼 순회하면 에러가 발생할 수 있음
    // 5칸 중에 실제 등록된 도서만 모아서 새로운 배열을 반환
    Book[] getBookDatas() {
        int count = 0;
        // 1. null이 아닌 도서의 갯수를 센다(count 이용)
        for (int i = 0; i < books.length; i++) {
            if (books[i] != null) {
                count++;
            }
