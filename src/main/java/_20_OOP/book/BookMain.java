...
// 서버 : 데이터를 운송(서빙)라는 것
// Main의 역할 = 사용자 UI(프론트엔드) + 컨트롤러(백엔드) : 사용자 입/출력, 프로그램 흐름만 제어
...
public class BookMain {
    public static void main(String[] args) {
        ...
        // BookMain의 main 메서드에서 생성 -> bookService -> bookRepository의 배열에 저장
    }

        // 사용자 입력을 받아줄 스캐너 생성
        Scanner scanner = new Scanner(System.in);
        // 서비스에 필요한 객체를 모두 생성
        Book[] books = new Book[5]; // 책 정보 저장할 배열
        // bookRepository가 books를 알게 됨
        BookRepository bookRepository = new BookRepository(books);
        // bookService가 bookRepository를 알게 됨
        ...
        while(true) {
            ...
            if ("1.".equals(selectMenu)) { // 1. 도서 등록
                // 객체로 포장해서 입력
                String title;
                String author;
            ...
            } else if ("2".equals((selectMenu))) { // 2. 전체 도서 조회
                bookService.printRegisterBooks();
