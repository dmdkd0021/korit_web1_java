...
public class Main {
    public static void main(String[] args) {
        /*
        1. 원시자료형(첫글자 소문자 시작)의 한계

        - 원시형은 객체가 아니므로
            1. 메서드를 가질 수 없음 : 외부에서 메서드를 정의해서 사용
            2. null 값을 가질 수 없다
            int[] scores = {80, 90, 30, 0, 50};
            score[3]의 값이 0인데, 초기화가 안 돼서 0인 건지 실제 점수가 0인 건지...
            -> null로 명시하면 조을 텐뎋...
         */

        // 원시자료형 -> 래퍼클래스(박싱)
        // int를 감싸느 래핑클래스 : Integer
        Integer integer1 = Integer.valueOf(90); //(Integer은 클래스로 나온 객체임)
        ...
        // jdk 설치하면 같이 설치되는 java.lang 패키지에 정의된 걸 사용하면 된다
        // double을 감싸는 래핑클래스 : Double
        // boolean을 갑싸는 래핑클래서 : Boolean
        // float -> Float
